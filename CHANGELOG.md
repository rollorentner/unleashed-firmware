## Main changes
- Current API: 88.3
* NFC: **Read MIFARE Ultralight AES (MF0AES20) via AES authentication + dictionary attack** - UL-AES is no longer identity-only: the reader now performs the AES-128 3-pass mutual authentication (DataProtKey), reads the protected memory, and recovers the key via an automatic dictionary attack (user + system dicts) or a manually entered key ("Unlock"), just like Ultralight-C; the recovered key is shown and saved with the dump (in real tag byte order), and site keys can be managed under Extra Actions -> MIFARE Ultralight AES Keys. Write a saved dump back to a card with "Write (Keep Key)" / "Write (Copy Key)" (user data pages, optionally the DataProtKey; config/lock pages are left untouched). Emulate a captured UL-AES so a reader can run the AES 3-pass authentication and read the AUTH0/PROT-gated memory (key pages never leak), including when secure messaging (CMAC) is enabled. Reads the 3 one-way counters (over secure messaging too), decodes the config (AUTH0/PROT/AUTH_LIM/etc.) in the info view, reveals the real UID on Random ID cards, and fully supports secure messaging (CMAC): reads, counter reads, and write-back are MAC-wrapped end-to-end (write-back auto-detects a secure-messaging card and switches transparently), and emulation MAC-wraps its own responses too (by @mishamyte)
